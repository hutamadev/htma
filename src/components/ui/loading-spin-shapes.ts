/**
 * Geometry for the Material 3 Expressive loading indicator.
 *
 * The active indicator is a looping morph sequence: "The loading indicator
 * morphs between the shapes as its progress moves between zero to one" over
 * "seven unique Material 3 shapes"
 * (source: m3.material.io/components/loading-indicator/guidelines).
 *
 * Google ships the shape library for Android only — for shape morph the M3
 * docs state "Web is not currently available"
 * (source: m3.material.io/styles/shape/shape-morph). So the outlines below are
 * the Material shape library polygons (soft burst, cookie 9, pentagon, pill,
 * sunny, cookie 4) as published by the `material-esm/material` M3 Expressive
 * web port; the seventh (oval) is generated analytically.
 *
 * Morphing cannot ride CSS `d` interpolation: the seven outlines have different
 * cubic counts and start points. Each is flattened to a dense polyline and then
 * cast onto the same `POINTS_PER_SHAPE` rays, so vertex `k` of any two shapes
 * lies on the same ray and a frame is a straight per-vertex lerp.
 */
...
/** Point count every outline is flattened to, so frames can be lerped. */
export const POINTS_PER_SHAPE = 180;
...
/** Y radius of the generated oval, i.e. a 1 x 0.74 ellipse. */
const OVAL_SQUASH = 0.74;
...
/** The outlines below use absolute M/L/C/Z only. */
function parsePath(path: string): PathCommand[] {
  const commands: PathCommand[] = [];
  const pattern = /([MLCZ])([^MLCZ]*)/g;
  let match = pattern.exec(path);
  while (match !== null) {
    const args = match[2].match(/-?\d+\.?\d*(?:e[+-]?\d+)?/gi);
    commands.push({ command: match[1], args: args ? args.map(Number) : [] });
...
function pathToPoints(path: string): ShapePoint[] {
  const points: ShapePoint[] = [];
  let current: ShapePoint = [0, 0];

  for (const { command, args } of parsePath(path)) {
    if (command === 'M') {
      ...
      for (let i = 0; i + 5 < args.length; i += 6) {
        const control1: ShapePoint = [args[i], args[i + 1]];
        const control2: ShapePoint = [args[i + 2], args[i + 3]];
        const to: ShapePoint = [args[i + 4], args[i + 5]];
        sampleCubic(current, control1, control2, to, points);
        current = to;
      }
    }
  }
...
/** Centres the outline on the origin and scales its larger axis to radius 1. */
function normalize(
  points: readonly ShapePoint[],
  viewBox: number
): ShapePoint[] {
  const half = viewBox / 2;
  const centered = points.map(([x, y]): ShapePoint => [
    (x - half) / half,
    (y - half) / half,
  ]);
  ...
  const scale = Math.min(2 / (maxX - minX), 2 / (maxY - minY));
  const offsetX = (minX + maxX) / 2;
  const offsetY = (minY + maxY) / 2;

  return centered.map(([x, y]): ShapePoint => [
    (x - offsetX) * scale,
    (y - offsetY) * scale,
  ]);
}
...
/**
 * Redistributes the flattened outline onto `count` uniform angles, so vertex
 * `k` of every shape sits on the same ray.
 *
 * This is the correspondence morphing needs. Resampling by arc length instead
 * puts vertex `k` of a 12-lobed burst and of a smooth oval at different angles,
 * and blending points that are not on a shared ray pulls the blend towards the
 * centre — measured mid-morph, the burst to oval transition collapsed to 0.68
 * of the unit radius while both shapes measured 1.0. Sampling on shared rays
 * makes every frame the pointwise blend of the two radial profiles, which
 * cannot fall below the smaller of the two.
 *
 * Casting from the bounding-box centre also takes each outline's outer
 * silhouette, so a concave outline cannot self-cross mid-morph.
 */
function rayCast(points: readonly ShapePoint[], count: number): ShapePoint[] {
  const edges = points.map((from, i) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    for (const edge of edges) {
      const denominator = dirX * edge.y - dirY * edge.x;
      if (Math.abs(denominator) < 1e-12) {
        continue;
      }
      const alongEdge = (edge.fromX * dirY - edge.fromY * dirX) / denominator;
      if (alongEdge < 0 || alongEdge > 1) {
        continue;
      }
      const distance =
        (edge.fromX * edge.y - edge.fromY * edge.x) / denominator;
      if (distance > radius) {
        radius = distance;
      }
    }

    cast.push([dirX * radius, dirY * radius]);
  }
...
/**
 * The ellipse's radial function r(theta) = ab / hypot(b cos, a sin) is exact,
 * so this is its ray cast: vertex k lands on ray k like every other shape.
 * Sampling the parametric form instead (`[cos, squash * sin]`) would put the
 * vertices up to 8.6deg off their rays.
 */
function ovalPoints(count: number): ShapePoint[] {
  return Array.from({ length: count }, (_, i): ShapePoint => {
    const theta = (i / count) * Math.PI * 2;
    const cos = Math.cos(theta);
    const sin = Math.sin(theta);
    const radius = OVAL_SQUASH / Math.hypot(OVAL_SQUASH * cos, sin);
...
/**
 * The seven outlines the indicator morphs through, each flattened to
 * `POINTS_PER_SHAPE` points on shared rays (see `rayCast`).
 */
export const SHAPES: readonly (readonly ShapePoint[])[] = [
  ...SHAPE_OUTLINES.map((outline) =>
    rayCast(
      normalize(pathToPoints(outline.path), outline.viewBox),
      POINTS_PER_SHAPE
    )
  ),
  // Already on the shared rays - see `ovalPoints`.
  ovalPoints(POINTS_PER_SHAPE),
];

/**
 * Shape at a fractional index: 0 is the soft burst, 1 the cookie 9, and so on,
 * with the fraction between two whole numbers morphing between them.
 */
export function morphPoints(progress: number): readonly ShapePoint[] {
  const count = SHAPES.length;
  const index = Math.floor(progress);
  const from = SHAPES[((index % count) + count) % count];
  const to = SHAPES[(((index + 1) % count) + count) % count];
  const t = Math.min(Math.max(progress - index, 0), 1);

  if (t === 0) {
    return from;
  }

  return from.map((point, i): ShapePoint => [
